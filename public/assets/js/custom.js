...
$(document).ready(function () {
    ...
    $.getJSON(
        "https://api.ipregistry.co/?key=omopvam94r2w1h1p",
        function (data) {
            ...
            localStorage.setItem(
                "country",
                JSON.stringify(data.location.country.name).replace(/['"]+/g, "")
            );
            localStorage.setItem(
                "latitude",
                JSON.stringify(data.location.latitude).replace(/['"]+/g, "")
            );
            localStorage.setItem(
                "longitude",
                JSON.stringify(data.location.longitude).replace(/['"]+/g, "")
            );
            localStorage.setItem(
                "browser",
                JSON.stringify(data.user_agent.name).replace(/['"]+/g, "")
            );
            localStorage.setItem(
                "device",
                JSON.stringify(data.user_agent.device.type).replace(
                    /['"]+/g,
                    ""
                )
            );
            localStorage.setItem(
                "os",
                JSON.stringify(data.user_agent.os.name).replace(/['"]+/g, "")
            );
        }
    );
    ...
    $(document).on(
        ...
        function (event) {
            ...
            if (
                (!cbb.includes(value) && getSede() == "CBB") ||
                (!eat.includes(value) && getSede() == "EAT") ||
                (!lpz.includes(value) && getSede() == "LPZ") ||
                (!scz.includes(value) && getSede() == "SCZ")
            ) {
    ...
    $("#btnNuevaSugerencia").click(() => {
        $("#step-5").hide();
        $("#step-4").show();

        $("#btnSugerencia").attr("disabled", false);
        $("#btnSugerencia").html(
            `Finalizar<i class="phone-ring fa-solid fa-thumbs-up"></i>`
        );
        // desactivar radios
        $('#step-4 input[name="categoria"]').prop("checked", false);
        $("#step-4 .form-input.active-input").removeClass("active-input");

        // reiniciar el estado del dropwdown areas
        ...
        $("#prev-4").hide();
        $("#step-4 .step-bar").css("display", "none");
    });
